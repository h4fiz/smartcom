...
import './RoomReservationForm.style.css';
...
class RoomReservationForm extends Component {
    ...
    loadRoom = () => {
        axios.post(webserviceurl + '/app_load_room.php', {
        ...
        {headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) => {
                ...
                if (result.status === "OK") {
                    this.setState({ rooms: result.records, type: result.records.length > 0 ? result.records[0].roomid : 0 });
					this.setState({ roomid: result.records[0].roomid});
                }
	...
	loadAddOn = () => {
        axios.post(webserviceurl + '/app_load_roomaddon.php', {
        },
        {headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) => {
                let result = response.data;
            ...
            buttons: [
              {
                  ...
                  onClick: () => {
                      ...
                      axios.post(webserviceurl + '/app_insert_roomreservationrequest.php', param,
                        {headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
                            .then((response) => {
                                let result = response.data;
	...
	checkIfExist=()=>{
        ...
        axios({
            method: 'post',
            url: webserviceurl + '/app_check_roomreservationdate.php',
            data: {
                checkin: checkin,
				checkout: checkout,
				roomid: this.state.roomid
	...
	updateQty=(newValue, roomaddonid)=>{
        let addon = this.state.addon;
        let theaddon = {};
		let tempcounter = 0;
        for(let i=0; i<addon.length;i++){
            if(addon[i].roomaddonid === roomaddonid){		
                theaddon = addon[i];
            ...
            <div className="main-container">
                ...
                <div className="service-center-form">
                    ...
                    <div className="form-container">
						...
						<div className="field-container">
                            <div className="field-label">{this.language.room}</div>
                            <div className="field-value">
                                <select value={this.state.roomid} onChange={(event)=>this.setState({roomid: event.target.value})}>
								{this.state.rooms.map((rooms,i)=>
                                        <option value={rooms.roomid}>{rooms.roomname}</option>
                                        )}
								</select>
                            </div>
                        </div>
						...
						<div className="field-container">
                            <div className="field-label bottom-bordered">{this.language.addon}</div>
                            <div className="field-value">
								{this.state.addon.map((item, i)=>
									<RoomAddOn key={i} roomaddonid={item.roomaddonid} roomaddonname={item.roomaddonname} qty={item.qty} updateQty={this.updateQty}  />
								)}
                            </div>
                        </div>
                    </div>
