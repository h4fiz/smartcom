...
import './TalentPage.style.css';

//import Picture from '../Component/pictureGallery';
import TalentListItem from '../../Component/TalentListItem/TalentListItem';
...
class TalentPage extends Component {
    ...
    constructor(props) {
        ...
        this.language = getLanguage(activelanguage, 'talentpage');
        this.loadtalent();
        this.waitForBridge();
    }
    ...
    goToDetail=(talent)=>{
        this.props.history.push('/talentdetail/'+talent.id);
    }

    renderList = () => {
        if (this.state.talent.list.length > 0) {
            return (
                <div className="body-section">
                    {this.state.talent.list.map((item, i) =>
                        <div key={i} onClick={()=>this.goToDetail(item)}>
                            <TalentListItem key={i} name={item.name} shortdesc={item.shortdesc} category={item.category} tags={item.tags} pic={item.talentpic[0]}  />
                        </div>
                    )}
                </div>
            )
